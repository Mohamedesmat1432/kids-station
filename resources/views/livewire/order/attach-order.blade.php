<div>
    <x-dialog-modal wire:model="attach_modal" submit="save()" method="PATCH">
                ...
                @foreach ($visitors as $key => $visitor)
                    <div class="grid md:grid-cols-5 md:gap-3">
                        ...
                        <div class="relative z-0 w-full mb-5 group">
                            ...
                            <x-input type="hidden" class="mt-1 block w-full" wire:model="visitors.{{ $key }}.type_id" />
                            <x-input type="text" class="mt-1 block w-full" value="{{ App\Models\Type::find($visitor['type_id'])->typeName->name ?? '' }}" />
                            <x-input-error for="visitors.{{ $key }}.type_id" class="mt-2" />
                        </div>
                        <div class="relative z-0 w-full mb-5 group">
                            <x-label for="serial" value="{{ __('site.serial') }}" />
                            <x-input type="text" class="mt-1 block w-full"
                                wire:model="visitors.{{ $key }}.serial" placeholder="{{ __('site.serial') }}"
                                disabled />
        ...
        <x-slot name="footer">
            ...
            <x-secondary-button class="mx-2" wire:click="$set('attach_modal',false)" wire:loading.attr="disabled">
                {{ __('site.cancel') }}
            </x-secondary-button>
        </x-slot>
    </x-dialog-modal>
